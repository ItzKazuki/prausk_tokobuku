        ...
        <div class="flex flex-col md:flex-row gap-8">
            <form action="{{ route('admin.books.store') }}" method="post" enctype="multipart/form-data" class="space-y-4">
                ...
                <fieldset class="fieldset">
                    ...
                    <input type="text" name="title" class="input w-full @error('title') border-error @enderror"
                        value="{{ old('title') }}" placeholder="Contoh: Laskar Pelangi" />
                    @error('title')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <fieldset class="fieldset">
                        ...
                        <input type="number" name="price" class="input w-full @error('price') border-error @enderror"
                            value="{{ old('price') }}" placeholder="50000" />
                        @error('price')
                            <span class="text-error text-sm">{{ $message }}</span>
                        @enderror
                    </fieldset>
                ...
                <fieldset class="fieldset">
                    ...
                    <textarea name="description" class="textarea w-full h-24 @error('description') border-error @enderror"
                        placeholder="Sinopsis buku...">{{ old('description') }}</textarea>
                    @error('description')
                        <span class="text-error text-sm">{{ $message }}</span>
                    @enderror
                </fieldset>
            ...
            <div class="w-full md:w-64">
                ...
                <div
                    ...
                    <div id="placeholder" class="text-center p-4 hidden">
